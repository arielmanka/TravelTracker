...
router.post('/limits', async (req, res) => {
  try {
    ...
    await db.run(
      `INSERT INTO limit_settings (country, max_days, rolling_period_days)
       VALUES (?, ?, ?)
       ON CONFLICT(country) 
       DO UPDATE SET max_days = excluded.max_days, rolling_period_days = excluded.rolling_period_days`,
      [country.trim(), parseInt(max_days), parseInt(rolling_period_days)]
    );
...
router.get('/countries/:countryId/cities', async (req, res) => {
  try {
    ...
    const cities = await db.all(
      'SELECT * FROM cities WHERE country_id = ? ORDER BY name ASC',
  ...
  try {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (!countryDatesMap[countryKey]) {
        countryDatesMap[countryKey] = new Set<string>();
      }
    ...
    for (const limit of limits) {
      ...
      const datesVisited = countryDatesMap[countryKey] || new Set<string>();
...
router.get('/forecast', async (req, res) => {
  try {
    ...
    const departureDate = (req.query.departureDate as string || '').trim();
    ...
    const limit = await db.get(
      'SELECT * FROM limit_settings WHERE LOWER(country) = LOWER(?)',
      [country]
    );
    if (!limit) {
    ...
    const datesSet   = datesMap[countryKey] || new Set<string>();
    ...
    for (const d of datesSet) {
      if (d >= windowStartStr && d <= departureDate) daysSpent++;
    }
...
router.get('/safe-return', async (req, res) => {
  try {
    const country  = (req.query.country as string || '').trim();
    const stayDays = Math.max(1, parseInt(req.query.stayDays as string || '1', 10));
    ...
    const countryKey = country.toLowerCase();
    const datesArr   = Array.from(datesMap[countryKey] || new Set<string>()).sort();
    ...
    for (let offset = 1; offset <= MAX_SCAN_DAYS; offset++) {
      ...
      for (const d of datesArr) {
        if (d >= winStartStr && d <= stayEndStr) existingInWindow++;
      }
 ...
 */
function buildCountryDatesMap(
  entries: any[],
  extendLastStayToDate?: string // YYYY-MM-DD; only extends if last entry country matches
  ...
  for (let i = 0; i < entries.length; i++) {
    const entry      = entries[i];
    const countryKey = entry.country.trim().toLowerCase();
    if (!map[countryKey]) map[countryKey] = new Set<string>();
