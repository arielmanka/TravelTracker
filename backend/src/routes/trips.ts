    ...
    try {
      ...
      if (destinations && Array.isArray(destinations)) {
        for (const dest of destinations) {
          ...
          await db.run(
            'INSERT INTO destinations (trip_id, country, city, entry_date, exit_date) VALUES (?, ?, ?, ?, ?)',
            [tripId, country, city, entry_date, exit_date]
          );
        }
      }
    ...
    try {
      ...
      if (destinations && Array.isArray(destinations)) {
        ...
        if (toDeleteIds.length > 0) {
          ...
          const receiptsToDelete = await db.all(
            `SELECT file_path FROM receipts WHERE destination_id IN (${toDeleteIds.join(',')})`
          );
        ...
        for (const dest of destinations) {
          ...
          if (id) {
            await db.run(
              'UPDATE destinations SET country = ?, city = ?, entry_date = ?, exit_date = ? WHERE id = ? AND trip_id = ?',
              [country, city, entry_date, exit_date, id, tripId]
            );
          } else {
            await db.run(
              'INSERT INTO destinations (trip_id, country, city, entry_date, exit_date) VALUES (?, ?, ?, ?, ?)',
        ...
        const receiptsToDelete = await db.all(
          'SELECT file_path FROM receipts WHERE destination_id IN (SELECT id FROM destinations WHERE trip_id = ?)',
          [tripId]
        );
        const uploadsDir = process.env.UPLOADS_DIR || './data/receipts';
        for (const r of receiptsToDelete) {
          const fp = path.join(uploadsDir, r.file_path);
