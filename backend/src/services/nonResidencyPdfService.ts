 ...
 */
export async function generateNonResidencyPDF(
  ...
  const outsideSegments = segments.filter(s =>
    s.country.trim().toLowerCase() !== targetKey &&
    (s.endDate >= pastYearStr || s.startDate >= pastYearStr)
  );
  const insideSegments = segments.filter(s =>
    s.country.trim().toLowerCase() === targetKey &&
    (s.endDate >= pastYearStr || s.startDate >= pastYearStr)
  );
  ...
  const insideDatesSet  = new Set<string>();
  const outsideDatesSet = new Set<string>();
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    while (current <= end) {
      ...
      if (d >= pastYearStr && d <= todayStr) {
        if (isInside) insideDatesSet.add(d);
  ...
  doc.fillColor(stmtColor).fontSize(10).font('Helvetica-Bold')
     .text(
       compliant
         ...
         : `ATTENTION: ${ownerName || 'The subject'} spent ${daysInTarget} day(s) in ${targetCountry} in the 365-day period — at or above the 183-day threshold.`,
  ...
  for (const seg of outsideSegments) {
    ...
    doc.fillColor('#475569').fontSize(9).font('Helvetica')
       .text(`Arrival: ${seg.entry_time}`, LEFT + 12, rowY + 15, { width: 200 });
    doc.fillColor('#475569').fontSize(9).font('Helvetica')
    ...
    if (seg.file_path && seg.file_path.trim() !== '') {
      const fullPath = path.join(uploadsDir, seg.file_path);
      if (fs.existsSync(fullPath) && fs.lstatSync(fullPath).isFile()) {
        try {
          if (doc.y > 600) doc.addPage();
          doc.image(fullPath, LEFT + 12, doc.y, { fit: [280, 140] });
          doc.y += 148;
        } catch {
          doc.fillColor('#ef4444').fontSize(8).font('Helvetica-Bold')
             .text(`[Cannot render proof image: ${seg.file_name}]`, LEFT + 12, doc.y, { width: WIDTH });
          doc.moveDown(0.4);
        }
      }
    }
  ...
  if (insideSegments.length > 0) {
    ...
    for (const seg of insideSegments) {
      ...
      doc.fillColor('#64748b').fontSize(8.5).font('Helvetica')
         .text(`${seg.entry_time}  |  ${seg.durationDays} day(s)  (${seg.startDate} → ${seg.endDate})`, LEFT + 12, rowY + 14, { width: WIDTH - 12 });
      doc.y = rowY + 34;
    }
  }
  ...
  doc.fillColor('#334155').fontSize(8.5).font('Helvetica')
     .text(
       ...
       `The data presented herein, including arrival dates, stay durations, and supporting receipt images, represents the subject's ` +
       ...
       `by a qualified tax professional before submission to any authority.`,
